...
def generate_subscription_invoice_pdf(payment_order):
    """Generate PDF for subscription payment."""
    buffer = io.BytesIO()
    ...
    story.append(Paragraph("Subscription Invoice", styles['Heading2']))
    story.append(Spacer(1, 20))
    
    # Company Info
    data = [['IntraView Technologies', 'support@intraview.app'], ['GSTIN: 29ABCDE1234F1Z5', '']]
    table = Table(data, colWidths=[6*cm, 6*cm])
    table.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'LEFT')]))
    story.append(table)
    
    # Bill To
    bill_data = [['Bill To:', payment_order.user.email], ['Subscription:', payment_order.subscription.plan.name]]
