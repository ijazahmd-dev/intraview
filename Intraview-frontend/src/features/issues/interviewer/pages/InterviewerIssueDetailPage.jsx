// src/features/issues/interviewer/pages/InterviewerIssueDetailPage.jsx
...
import {
  ...
  selectInterviewerSelectedIssue,
  selectInterviewerDetailLoading,
  selectInterviewerDetailError,
  ...
  if (status === ISSUE_STATUS.WAITING_FOR_RESPONSE) {
    return (
      <div className="rounded-xl bg-orange-50 border border-orange-100 px-4 py-3 flex items-start gap-3 text-sm text-orange-700">
        ...
        <span>
          Our team needs additional information from you. Please check your
  ...
  if (status === ISSUE_STATUS.REJECTED) {
    return (
      <div className="rounded-xl bg-red-50 border border-red-100 px-4 py-3 flex items-start gap-3 text-sm text-red-700">
        ...
        <span>
          Your complaint was reviewed and was not upheld. If you believe this is
  ...
  if (status === ISSUE_STATUS.ACTION_TAKEN) {
    return (
      <div className="rounded-xl bg-purple-50 border border-purple-100 px-4 py-3 flex items-start gap-3 text-sm text-purple-700">
        <CheckCircle2 className="w-4 h-4 mt-0.5 flex-shrink-0" />
        <span>
          Our team has reviewed your report and taken appropriate action against
          the candidate.
        </span>
      </div>
    );
...
const InterviewerIssueDetailPage = () => {
  const { issueId } = useParams();
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const issue = useSelector(selectInterviewerSelectedIssue);
  const loading = useSelector(selectInterviewerDetailLoading);
  const error = useSelector(selectInterviewerDetailError);

  useEffect(() => {
    dispatch(fetchInterviewerIssueDetail(issueId));
    return () => {
      dispatch(clearInterviewerSelectedIssue());
    };
  }, [dispatch, issueId]);
  ...
  if (
    ...
    timeline.push({
      ...
      title: "Under Review",
      subtitle: "Our moderation team is reviewing your report.",
      date: null,
    });
  }
  ...
  if (issue.status === ISSUE_STATUS.WAITING_FOR_RESPONSE) {
    timeline.push({
      ...
      subtitle: "The team has requested additional details.",
      date: null,
    });
  }
  ...
  if (issue.status === ISSUE_STATUS.ACTION_TAKEN) {
    timeline.push({
      ...
      title: "Action Taken",
      subtitle: "Appropriate action has been taken against the candidate.",
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-4">
        ...
        {/* Back */}
        <button
          onClick={() => navigate("/interviewer/dashboard/my-issues")}
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          ...
          <InfoRow
            icon={User}
            label="Raised By (You)"
            value={issue.raised_by_name}
          />
          <InfoRow
            icon={User}
            label="Against Candidate"
            value={issue.against_user_name}
          />
          <InfoRow
            icon={Calendar}
            label="Reported On"
        ...
        {/* Presence data — useful for no-show reports */}
        {(issue.candidate_presence_minutes != null ||
          issue.interviewer_presence_minutes != null) && (
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              <h2 className="text-sm font-semibold text-gray-700 mb-2">
              ...
              {issue.interviewer_presence_minutes != null && (
                <InfoRow
                  icon={Clock}
                  label="Your Presence"
                  value={`${issue.interviewer_presence_minutes} min`}
                />
              )}
              {issue.candidate_presence_minutes != null && (
                <InfoRow
                  icon={Clock}
                  label="Candidate Presence"
                  value={`${issue.candidate_presence_minutes} min`}
                />
              )}
            </div>
          )}
...
export default InterviewerIssueDetailPage;