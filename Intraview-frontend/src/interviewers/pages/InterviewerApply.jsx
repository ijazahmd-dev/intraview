...
export default function InterviewerApply() {
  const dispatch = useDispatch();
  const { loading, status, existingApplication, loadingExistingApplication } =
    useSelector((s) => s.interviewer);

  const [activeStep, setActiveStep] = useState(0);
  const [formData,   setFormData]   = useState(EMPTY_FORM);

  // ── On mount: fetch existing application so we can pre-populate ───────────
  useEffect(() => {
    dispatch(fetchExistingApplication());
  }, [dispatch]);

  // ── Pre-populate form when existing (rejected) application is loaded ───────
  useEffect(() => {
    if (!existingApplication) return;
    // Only pre-populate if the application was rejected (re-apply scenario)
    ...
    setFormData({
      ...
      // File fields stay null — if user doesn't select a new file the backend
      // keeps the existing file. We show the current filename in the UI.
      resume:          null,
      certifications:  null,
      additional_docs: null,
      // Pass existing file names so StepDocuments can display "Current: …"
  ...
  const handleSubmit = async () => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      // Skip internal _existing_* metadata keys — not sent to the server
      ...
      // null / empty string file fields are omitted → backend keeps existing file
    });
  ...
  // Show a loading spinner while we fetch the existing application
  if (loadingExistingApplication) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 flex items-center justify-center">
        <div className="text-slate-500 text-sm">Loading your application…</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 py-10">
      <div className="max-w-5xl mx-auto px-4">
        <h1 className="text-3xl md:text-4xl font-semibold text-slate-800 mb-2">
          {isReapplying
            ? "Re-apply to Become an Interviewer"
            : "Apply to Become an Interviewer"}
        </h1>
        <p className="text-slate-500 mb-2">
          {isReapplying
            ? "Your previous application was rejected. Update your details below and resubmit."
            : "Fill out your details and submit credentials for review. We'll get back to you within 5–7 business days."}
        </p>
        ...
        {isReapplying && existingApplication?.rejection_reason && (
          <div className="mb-6 rounded-xl border border-red-200 bg-red-50 px-5 py-4 text-sm text-red-800">
            ...
            <p>{existingApplication.rejection_reason}</p>
          </div>
        )}
        ...
        <div className="grid md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-800 mb-3">Application Tips</h3>
              <ul className="text-xs text-slate-600 space-y-1">
                <li>• Be thorough and accurate with all information.</li>
                <li>• Highlight your unique interviewing experience.</li>
                <li>• Upload a professional, up-to-date resume.</li>
