...
import { ArrowLeft, Calendar, Clock, Coins, CheckCircle } from 'lucide-react';
...
const CalendarBookingPage = () => {
  ...
  const handleBookClick = (slot) => {
    const slotCost = slot.token_cost || profile?.base_session_rate || 10;
    if (tokenBalance < slotCost) {
      toast.error(`Need ${slotCost} tokens (you have ${tokenBalance})`);
      return;
    }
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      await Promise.all([fetchTokenBalance(), fetchCalendarEvents()]);
      closeAll();
      setAvailability([]);
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        ...
        >
          ...
          {isAvailable && (
            <div className={`text-xs mt-1 font-semibold px-2 py-0.5 rounded-full bg-white/80 backdrop-blur-sm shadow-sm ${isSelected ? 'text-emerald-900' : 'text-emerald-700'
              }`}>
              {capacity} slots
            </div>
          )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50">
            ...
            <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
              {selectedDate && availability.length === 0 && !loadingSlots ? (
                ...
                availability.map((slot) => {
                  ...
                  const slotCost = slot.token_cost || profile?.base_session_rate || 10;
                  const canBook = capacityAvailable && tokenBalance >= slotCost;
      ...
      <BookingSummaryModal
        ...
        tokenCost={selectedSlot?.token_cost || profile?.base_session_rate || 10}
