...
const TabNavigation = ({ activeTab, onTabChange }) => {
  const tabs = [
    { id: 'overview', label: 'Overview', icon: LayoutGrid, description: 'Profile info & links' },
    { id: 'preferences', label: 'Preferences', icon: Heart, description: 'Interview preferences' },
    { id: 'feedback', label: 'Feedback', icon: BarChart3, description: 'Interview feedback' },
    { id: 'tokens', label: 'Tokens', icon: Activity, description: 'Subscription & tokens' },
  ];

  return (
    <div style={{
      background: C.white, borderRadius: '20px', border: `1px solid ${C.grayBorder}`,
      boxShadow: '0 2px 12px rgba(0,0,0,0.05)', overflow: 'hidden', marginBottom: '24px',
          ...
          return (
            <button
              ...
              style={{
                ...
                border: 'none', background: isActive ? C.tealLight : C.white,
                cursor: 'pointer', transition: 'all 0.15s', fontFamily: '"DM Sans", sans-serif',
                borderBottom: `2.5px solid ${isActive ? C.teal : 'transparent'}`,
                borderRight: idx < 3 ? `1px solid ${C.grayBorder}` : 'none',
              }}
            >
              <Icon size={18} style={{ margin: '0 auto 4px', display: 'block', color: isActive ? C.teal : C.textMuted }} />
              <p style={{ margin: 0, fontSize: '12px', fontWeight: 700, color: isActive ? C.teal : C.text }}>
                {tab.label}
              </p>
              <p style={{ margin: '2px 0 0', fontSize: '10px', color: C.textMuted, display: 'none' }}>
                {tab.description}
              </p>
            </button>
          );
...
// ─── Main ProfilePage ──────────────────────────────────────────────────────────
const ProfilePage = () => {
  const { candidateProfile, isLoadingProfile, profileError } = useProfileData();
  const location = useLocation();
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const activeTab = useSelector((state) => state.profile?.activeTab || 'overview');
  ...
  useEffect(() => {
    if (profileError) {
      toast.error(typeof profileError === 'string' ? profileError : 'Failed to load profile');
    }
  }, [profileError]);
  ...
  if (isLoadingProfile && !candidateProfile) {
    return (
      <div style={{
        minHeight: '100vh', background: C.gray, display: 'flex',
        alignItems: 'center', justifyContent: 'center', fontFamily: '"DM Sans", sans-serif',
      }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
          <div style={{
            width: '48px', height: '48px', borderRadius: '50%',
            border: `3px solid ${C.tealBorder}`, borderTopColor: C.teal,
            animation: 'pp-spin 0.8s linear infinite',
          }} />
          <p style={{ color: C.textMuted, fontWeight: 500, fontSize: '14px' }}>Loading your profile…</p>
        </div>
  ...
  if (!candidateProfile) {
    return (
      <div style={{
        ...
        <div style={{
          background: C.white, borderRadius: '24px', boxShadow: '0 8px 40px rgba(0,0,0,0.10)',
          border: `1px solid ${C.grayBorder}`, padding: '40px', textAlign: 'center', maxWidth: '420px', width: '100%',
        }}>
          <h1 style={{ fontSize: '22px', fontWeight: 700, color: C.dark, margin: '0 0 8px' }}>Profile Not Found</h1>
  ...
  return (
    <>
      ...
      `}</style>
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: C.gray, fontFamily: '"DM Sans", sans-serif' }}>
        <CandidateNavbar />

        <div style={{ flex: 1, padding: '32px 16px', maxWidth: '1280px', margin: '0 auto', width: '100%' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '20px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div style={{
                      ...
                      <p style={{ margin: '0 0 12px', fontSize: '13px', fontWeight: 700, color: C.dark }}>Quick Actions</p>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {[
                          { label: 'Manage Resume', icon: FileText, to: '/candidate/resume' },
                          { label: 'Edit Preferences', icon: Heart, to: '/candidate/profile?tab=preferences' },
                          { label: 'Account Settings', icon: Settings, to: '/candidate/settings' },
                        ].map((item) => {
                          ...
                          return (
                            <button
                              ...
                              style={{
                                ...
                                padding: '10px 14px', borderRadius: '12px', border: `1px solid ${C.grayBorder}`,
                                background: C.gray, cursor: 'pointer', transition: 'all 0.12s',
                                fontSize: '13px', fontWeight: 500, color: C.text,
                                fontFamily: '"DM Sans", sans-serif',
                              }}
                              onMouseEnter={e => { e.currentTarget.style.background = C.tealLight; e.currentTarget.style.borderColor = C.tealBorder; }}
                              onMouseLeave={e => { e.currentTarget.style.background = C.gray; e.currentTarget.style.borderColor = C.grayBorder; }}
                            >
